...
export const recommendCourses = onCall({ region, secrets: [openRouteServiceKey, openRouterKey], timeoutSeconds: 90, memory: '512MiB' }, async request => {
  ...
  const results = await Promise.allSettled(candidates.map(async (candidate, index) => { const routePoints = [start!, ...candidate.waypoints, ...(tripType === 'round' ? [start!] : [])]; const verified = await callOrs(routePoints); const climbRate = verified.elevationM / Math.max(verified.distanceKm, 1); const recommended = candidate.title.startsWith('[수변 추천]'); const cleanTitle = candidate.title.replace(/^\[수변 추천\]\s*/, ''); const waterwayName = recommended ? cleanTitle.match(/^([^\s]+(?:천|강|호|수변))/)?.[1] ?? cleanTitle.split(/\s+/)[0] : undefined; return { id: `candidate-${index + 1}`, title: cleanTitle, summary: candidate.summary, startName, recommended, waterwayName, ...verified, climbRate: Math.round(climbRate * 10) / 10, distanceDifferenceKm: Math.round(Math.abs(verified.distanceKm - distanceKm) * 10) / 10, verified: true } }))
  const fulfilled = results.flatMap(result => result.status === 'fulfilled' ? [result.value] : [])
  if (!fulfilled.length) throw new HttpsError('unavailable', '현재 조건으로 검증 가능한 코스를 찾지 못했습니다. 출발지나 거리를 바꿔 다시 시도해 주세요.')
  return { candidates: fulfilled.sort((a, b) => Number(b.recommended) - Number(a.recommended) || Number(a.distanceDifferenceKm) - Number(b.distanceDifferenceKm)).slice(0, 3) }
})
