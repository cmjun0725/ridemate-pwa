...
4. 저장소 **Actions** → `Deploy web app to GitHub Pages` → **Run workflow**를 실행하거나 새 커밋을 push합니다.
5. 빌드 작업과 deploy 작업이 모두 초록색인지 확인합니다.
...
- 웹 푸시는 HTTPS에서만 동작합니다. GitHub Pages 주소는 HTTPS이므로 조건을 만족합니다.
...
1. 배포된 [RideMate](https://cmjun0725.github.io/ridemate-pwa/)에 로그인합니다.
...
- `Firebase 웹 푸시 인증키(VAPID) 설정이 아직 필요합니다.`: GitHub Secret 이름 또는 Pages 재배포 여부를 확인합니다.
